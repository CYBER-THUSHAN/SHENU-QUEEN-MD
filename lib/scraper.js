...
if (fs.existsSync('./lib/Shenu/' + Config.LANG + '.json')) {	
	//log(pint('Loading ' + Config.LANG + ' language...', '.'));	
	var json = JSON.parse(fs.readFileSync('./lib/Shenu/' + Config.LANG + '.json'));	
} else {	
	var json = JSON.parse(fs.readFileSync('./lib/Shenu/SHENU-QUEEN.json'));	
}	
...
async function fbdl(Link) {
	...
	const Form = {
		...
		await axios(`https://fdownloader.net/`, {
			method: "POST",
			...
			axios({
				url: "https://fdownloader.net/downloader",
				method: "POST",
				data: new URLSearchParams(Object.entries({url: Url, format: "", token: token})),
				headers: {
					"accept": "*/*",
